...
simardeep_102003559_state = [[1, 2, 3], [8, -1, 4], [7, 6, 5]]
...
def calculate_heuristic(s):
    ...
    for i in range(0, 3):
        for j in range(0, 3):
            for k in range(0, 3):
                for l in range(0, 3):
                    if(s[i][j] != -1 and s[i][j] == simardeep_102003559_state[k][l]):
...
def print_final():
    state = tuple(list(chain.from_iterable(simardeep_102003559_state)))
    list1 = [state]
...
while(len(queue) > 0):
    ...
    if(queue[0] == simardeep_102003559_state):
        break
